...
# download URLs (override via custom-config.sh / install.sh)
export PANEL_RELEASE_TAG=${PANEL_RELEASE_TAG:-"1.11.11"}
export WINGS_RELEASE_TAG=${WINGS_RELEASE_TAG:-"v1.11.11"}
...
welcome() {
  ...
  if [ "$1" == "panel" ]; then
    output "Panel build: $PTERODACTYL_PANEL_VERSION"
  elif [ "$1" == "wings" ]; then
    output "Wings build: $PTERODACTYL_WINGS_VERSION"
  fi
  print_brake 70
}
...
get_latest_versions() {
  if [ "$USE_CUSTOM_BUILDS" == true ]; then
    output "Using custom Panel and Wings builds from dens09/Ptero-Install."
    PTERODACTYL_PANEL_VERSION="${PANEL_RELEASE_TAG:-custom} (panel.tar.gz)"
    PTERODACTYL_WINGS_VERSION="${WINGS_RELEASE_TAG:-custom} (wings_linux_\$ARCH)"
    return
  fi
